...
; Include all Tempo modules (64-bit compatible)
...
kernel_main:
    ...
    ; Enable interrupts for scheduler (after proper IDT setup)
    ; sti  ; Commented out until IDT is properly configured
...
init_shell_system:
    ...
    mov rdi, shell_task_entry
    mov rsi, 10         ; 10 ticks = 10ms
    call schedule_task
...
; Include new 64-bit assembly modules
%include "kernel/keyboard_driver64.asm"
%include "kernel/timer64.asm"
%include "kernel/atomic_shell64.asm"
...
section .data
welcome_msg: db 'AtomicOS v5.2.1 - 64-bit Deterministic OS + Tempo Security', 0
tempo_msg:   db 'Pure 64-bit! Native macOS Tempo Compiler Support!', 0
